...
Requires:           bm-jdk = 17.0.4.1+1-bluemind12, bm-kafka = 3.1.0-bluemind21
...
if [ $1 -gt 1 ]; then
    # Upgrade
    [ -d /run/systemd/system ] && systemctl stop bm-forest-node
fi
...
if [ $1 -eq 1 ]; then
    # Upgrade
    [ -d /run/systemd/system ] && systemctl start bm-forest-node
fi
