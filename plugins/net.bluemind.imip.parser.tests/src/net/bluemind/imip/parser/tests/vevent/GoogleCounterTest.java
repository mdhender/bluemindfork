...
public class GoogleCounterTest extends IMIPTestCase {

	public void testBlueMindReply() throws Exception {
		Message m = parseData("google_counter.eml");
		assertNotNull(m);

		IMIPInfos infos = IMIPParserFactory.create().parse(m);
		assertNotNull(infos);
		assertEquals(ITIPMethod.COUNTER, infos.method);
