...
public class ScheduleInbox extends DavResource {
	...
	static {
		...
		props.add(CalendarFreeBusySet.NAME);
		props.add(DisplayName.NAME);
		props.add(GetCTag.NAME);
		props.add(ScheduleDefaultCalendarUrl.NAME);
		props.add(SupportedCalendarComponentSet.NAME);
		props.add(CalendarAvailability.NAME);
		props.add(SyncToken.NAME);

		properties = ImmutableSet.copyOf(props);

		types = ImmutableList.of(Types.COL, Types.SCHED_INBOX);
	}

	public ScheduleInbox(String path) {
		super(path, ResType.SCHEDULE_INBOX);
	}
