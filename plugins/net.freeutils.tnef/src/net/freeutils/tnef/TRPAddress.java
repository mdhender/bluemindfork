...
/**
 * The <code>TRPAddress</code> class encapsulates a TRP-formatted address used
 * in some TNEF message attributes.
 *
...
public class TRPAddress extends Address {
	 ...
	 */
	public static final int trpidNull = 0x0000, trpidUnresolved = 0x0001, trpidResolvedNSID = 0x0002,
			trpidResolvedAddress = 0x0003, trpidOneOff = 0x0004, trpidGroupNSID = 0x0005, trpidOffline = 0x0006,
			trpidIgnore = 0x0007, trpidClassEntry = 0x0008, trpidResolvedGroupAddress = 0x0009;
	 ...
	 *
	 * @param data
	 *            the TNEF stream to parse address from
	 * @throws IOException
	...
	public TRPAddress(RawInputStream data) throws IOException {
		...
		// skip u16
		data.readU16(); // 2*sizeof(TRP) + display length (with padding) +
						// address length
	 ...
	 *
	 * @param ID
	 *            the ID of the TRPAddress (from trpid* constants)
	 * @param displayName
	 *            the display name associated with address
	 * @param type
	 *            the address type
	 * @param address
	 *            the address
	...
	public String toString() {
		return new StringBuffer().append(TNEFUtils.getConstName(this.getClass(), "trpid", getID())).append(": ")
