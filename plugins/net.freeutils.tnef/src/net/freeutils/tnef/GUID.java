...
public class GUID {
	 ...
	 * 
	 * @param guid
	 *            the GUID value as a hex string (with optional canonical
	 *            dashes)
	 * @throws IllegalArgumentException
	 *             if the given string does not contain a valid GUID
	 */
	 ...
	 *            the GUID value
	 * @throws IllegalArgumentException
	 *             if the given array does not contain a valid GUID
	 */
	 ...
	 * 
	 * Valid GUIDs consist of 32 case insensitive hex digits (representing 16
	 * bytes), with or without the separating dashes in the appropriate
	 * positions.
	 * 
	 * @param guid
	 *            the GUID value
	...
	public static String canonize(String guid) {
		...
		if (len != 32 && len != 36)
			throw new IllegalArgumentException(
					"invalid GUID: " + guid + " (string length must be 32 without dashes, or 36 with dashes)");
		...
		for (int src = 0, dst = 0; src < len; src++, dst++) {
			...
			if (dst == 8 || dst == 13 || dst == 18 || dst == 23) {
				...
				if (len == 36) {
					if (c != '-')
						throw new IllegalArgumentException(
								"invalid GUID: " + guid + " ('-' expected at position " + src + ")");
					c = guid.charAt(++src);
				}
			}
			if (c < '0' || c > 'f' || (c > '9' && c < 'a'))
				throw new IllegalArgumentException(
						"invalid GUID: " + guid + " (invalid hex character at position " + src + ": '" + c + "')");
			chars[dst] = c;
		}
