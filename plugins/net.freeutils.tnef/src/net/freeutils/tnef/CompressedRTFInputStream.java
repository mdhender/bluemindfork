...
/**
 * The <code>CompressedRTFInputStream</code> class provides decompression of
 * compressed RTF data.
 *
...
public class CompressedRTFInputStream extends InputStream {
	...
	protected static final int MAGIC_COMPRESSED = 0x75465a4c, MAGIC_UNCOMPRESSED = 0x414c454d;
	 ...
	 * @param off
	 *            the offset of buf at which the CRC32 calculation will start
	 * @param len
	 ...
	 * algorithm than this stream's algorithm. It trades memory footprint (twice
	 * the size of the entire output is kept in memory) for speed (an order of
	 * magnitude faster than the stream version).
	 * <p>
	 * It also serves as a concise reference implementation for the
	...
	public static byte[] decompressRTF(byte[] src) {
		...
		in += 4;
		// note: CRC must be validated only for compressed data (and includes
		// padding)
			...
			try {
				while (true) {
					...
					if ((flags & 1) == 0) {
						...
						// read reference: 12-bit offset (from block start) and
						// 4-bit length
						...
						offset = (offset << 4) | (length >>> 4); // the offset
																	...
																	// block
																	// start
						length = (length & 0xF) + 2; // the number of bytes to
														// copy
						// the decompression buffer is supposed to wrap around
						// back
						// to the beginning when the end is reached. we save the
						// need for such a buffer by pointing straight into the
						// data
						// buffer, and simulating this behaviour by modifying
						// the
						// pointers appropriately.
						offset = out & 0xFFFFF000 | offset; // the absolute
															// offset in array
						if (offset >= out) {
							if (offset == out)
						...
						// note: can't use System.arraycopy, because the
						// referenced
	...
	protected void init() throws IOException {
		// get header fields
		compressedSize = (int) TNEFUtils.getU32(buf, 0) - 12; // includes header
																// except size
																// field itself
	 ...
	 * 
	 * @return true if more data was read, false if there is no more data
	 *         available
	 * @throws IOException
	 *             if an error occurs
	 */
	protected boolean moreCompressed() throws IOException {
		...
		while (true) {
			...
			if ((flags & 1) == 0) {
				...
				// read reference: 12-bit offset (from block start) and 4-bit
				// length
				int offset = buf[bufstart++] & 0xFF;
				int length = buf[bufstart++] & 0xFF;
				offset = (offset << 4) | (length >>> 4); // the offset from
															// block start
			...
			if (((DICT_SIZE + dictstart - out) & 0x0FFF) <= 17)
				return true; // dict is full - may not be room for another
								// reference
		}
		// verify source size
		if (compressedSize < 0)
