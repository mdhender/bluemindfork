...
public class BasicStoreTests extends LoggedTestCase {
	...
	public void testAppend() throws Exception {
		try (StoreClient sc = newStore(false)) {
			...
			assertTrue(uid > 0);
			// Date must be greater than the first leap year after 1970 (i.e. 1972)
			Date d = new Date(632966400000L);
	...
	public void testNoopSpeed() {
		try (StoreClient sc = newStore(false, 5)) {
			int count = 100000;
			...
			System.err.println(threadDump.toString());
			assertTrue(false);
		}
	}
	...
	public void testAppendPerf() throws Exception {
		try (StoreClient sc = newStore(false)) {
			...
			time = System.currentTimeMillis() - time;
			System.err.println(CNT + " IMAP append done in " + (time / 1000) + "s. Performing at "
					+ (CNT * 1000.0 / time) + "/sec");
		}
	}
	...
	public void testUidCopy() throws Exception {
		try (StoreClient sc = newStore(false)) {
			...
			System.out.println(
					"uidCopy for " + toCopy.size() + " uids took " + nstime + "ns (" + (nstime / 1000000) + "ms)");
		}
	}
