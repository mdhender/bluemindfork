...
public final class DecoderUtils {
	...
	private static final int ALL_MONTH_BITS = JAN_BIT | FEB_BIT | MAR_BIT | APR_BIT | MAY_BIT | JUN_BIT | JUL_BIT
			| AUG_BIT | SEP_BIT | OCT_BIT | NOV_BIT | DEC_BIT;
	...
	public static int decodeZone(char zoneDeterminent, char zoneDigitOne, char zoneDigitTwo, char zoneDigitThree,
			char zoneDigitFour) throws DecodingException {
	...
	private static DecodingException createTimeZoneException(char zoneDeterminent, char zoneDigitOne, char zoneDigitTwo,
			char zoneDigitThree, char zoneDigitFour) {
		return new DecodingException("Expected time-zone but was " + zoneDeterminent + zoneDigitOne + zoneDigitTwo
	...
	private static boolean isInvalidZone(char zoneDeterminent, char zoneDigitOne, char zoneDigitTwo,
		...
		if (zoneDeterminent == '+' || zoneDeterminent == '-') {
			result = !(isSimpleDigit(zoneDigitOne) && isSimpleDigit(zoneDigitTwo) && isSimpleDigit(zoneDigitThree)
					&& isSimpleDigit(zoneDigitFour));
		} else {
			result = true;
		}
		return result;
	 ...
	 * @throws DecodingException
	 */
	public static int decodeYear(final char milleniumChar, final char centuryChar, final char decadeChar,
			final char yearChar) throws DecodingException {
		return (decodeDigit(milleniumChar) * 1000) + (decodeDigit(centuryChar) * 100) + (decodeDigit(decadeChar) * 10)
	...
	public static int decodeFixedDay(final char dayHigh, final char dayLow) throws DecodingException {
		...
		throw new DecodingException("Expected SP, 0, 1, 2, or 3 but was " + dayHigh);
	}
	...
	public static int decodeDigit(char character) throws DecodingException {
		...
		if (result < 0 || result > 9) {
			throw new DecodingException("Expected a digit but was '" + character + "'");
		}
		return result;
	}
}
