...
public class NotTruncatedMime implements ISyncOptionsProvider {

	@Override
	public void setSyncOptions(Element options) {
		DOMUtils.createElementAndText(options, "MIMESupport", MIMESupport.Always.xmlValue());
		DOMUtils.createElementAndText(options, "MIMETruncation", MIMETruncation.TRUNCATE_NOTHING.xmlValue());
		Element bodyPreference = DOMUtils.createElement(options, "AirSyncBase:BodyPreference");
		DOMUtils.createElementAndText(bodyPreference, "Type", BodyType.MIME.xmlValue());
