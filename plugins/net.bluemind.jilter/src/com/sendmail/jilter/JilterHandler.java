...
public interface JilterHandler {
	/**
	 * Flag for {@link #getSupportedProcesses getSupportedProcesses} to indicate
	 * that {@link #connect connect} is supported.
	 */
	public static final int PROCESS_CONNECT = 0x0001;

	/**
	 * Flag for {@link #getSupportedProcesses getSupportedProcesses} to indicate
	 * that {@link #helo helo} is supported.
	 */
	public static final int PROCESS_HELO = 0x0002;

	/**
	 * Flag for {@link #getSupportedProcesses getSupportedProcesses} to indicate
	 * that {@link #envfrom envfrom} is supported.
	 */
	public static final int PROCESS_ENVFROM = 0x0004;

	/**
	 * Flag for {@link #getSupportedProcesses getSupportedProcesses} to indicate
	 * that {@link #envrcpt} is supported.
	 */
	public static final int PROCESS_ENVRCPT = 0x0008;

	/**
	 * Flag for {@link #getSupportedProcesses getSupportedProcesses} to indicate
	 * that {@link #header header} is supported.
	 */
	public static final int PROCESS_HEADER = 0x0010;

	/**
	 * Flag for {@link #getSupportedProcesses getSupportedProcesses} to indicate
	 * that {@link #body body} is supported.
	 */
	 ...
	 *            <code>getpeername()</code> call on the SMTP socket.
	 * @param properties
	 *            Any properties (macros) received from the MTA.
	 * @return <code>SMFIS_</code> return codes from {@link JilterStatus}.
	 *         <b>NOTE:</b> The MTA will currently ignore any custom values
	 *         (values other than <code>SMFIS_</code> values). Specifically,
	 *         values created with {@link JilterStatus#makeCustomStatus
	 *         JilterStatus.makeCustomStatus} will not be honored.
	 */
	JilterStatus connect(String hostname, InetAddress hostaddr, Properties properties);

	/**
	 * Handle the HELO/EHLO command. Called whenever the client sends a
	...
	/**
	 * Handle the envelope FROM command. Called once at the beginning of each
	 * message, before <code>envrcpt</code>.
	 * 
	 * @param argv
	 *            An array of SMTP command arguments. <code>argv[0]</code> is
	...
	/**
	 * Handle the envelope RCPT command. Called once per recipient, hence one or
	 * more times per message, immediately after {@link #envfrom envfrom}.
	 * 
	 * @param argv
	 *            An array of SMTP command arguments. <code>argv[0]</code> is
	...
	/**
	 * Handle a message header. Called zero or more times between
	 * {@link #envrcpt envrcpt} and {@link #eoh eoh}, once per message header.
	 * 
	 * @param headerf
	 *            Header field name.
	 * @param headerv
	 ...
	 *            space). The trailing line terminator (CR/LF) is removed.
	 * @return <code>SMFIS_</code> return codes from {@link JilterStatus}.
	 */
	JilterStatus header(String headerf, String headerv);

	/**
	 * Handle the end of message headers. Called once after all headers have
	 ...
	 * 
	 * @param bodyp
	 *            This block of body data.
	 * @return <code>SMFIS_</code> return codes from {@link JilterStatus}.
	 */
	JilterStatus body(ByteBuffer bodyp);

	/**
	 * End of a message. Called once after all calls to {@link #body body} for a
	 * given message.
	 * 
	 * @param eomActions
	 *            Interface for effecting message changes.
	 * @param properties
	 *            Any properties (macros) received from the MTA.
	 * @return <code>SMFIS_</code> return codes from {@link JilterStatus}.
	 */
	...
	/**
	 * Handle the current message being aborted. Called at any time during
	 * message processing (i.e. between some message-oriented routine and
	 * {@link #eom eom}).
	 * 
	 * @return <code>SMFIS_</code> return codes from {@link JilterStatus}.
	 */
	...
	/**
	 * The current connection is being closed. Always called at the end of each
	 * connection.
	 * 
	 * @return <code>SMFIS_</code> return codes from {@link JilterStatus}.
	 */
