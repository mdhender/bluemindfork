...
public class StripAccents {
	...
	public static String strip(String value) {
		...
		// Worst-case length required: 4 * value.length()
		char[] output = new char[4 * value.length()];
		foldToASCII(input, 0, output, 0, value.length());
		return new String(output);
	}

	private static final void foldToASCII(char[] input, int inputPos, char[] output, int outputPos, int length) {
