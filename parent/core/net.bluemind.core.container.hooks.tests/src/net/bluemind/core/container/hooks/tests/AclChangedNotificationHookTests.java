...
import net.bluemind.core.container.hooks.aclchangednotification.AclWithState;
...
public class AclChangedNotificationHookTests extends AbstractHookTests {
	...
	public void testOwnerNotification_add_acl() throws ServerFault {
		...
		assertTrue(mail.diff().stream().anyMatch(d -> d.entry.subject.equals(test1.uid)
				&& d.status == AclWithState.AclStatus.ADDED && d.entry.verb == Verb.Write));
		assertTrue(mail.diff().stream().anyMatch(d -> d.entry.subject.equals(test2.uid)
				&& d.status == AclWithState.AclStatus.ADDED && d.entry.verb == Verb.Read));
	}
	...
	public void testOwnerNotification_remove_acl() throws ServerFault {
		...
		assertEquals("calendar", mail.containerType());
		assertFalse(mail.diff().stream().anyMatch(d -> d.entry.subject.equals(test1.uid)
				&& d.status == AclWithState.AclStatus.ADDED && d.entry.verb == Verb.Write));
		assertTrue(mail.diff().stream().anyMatch(d -> d.entry.subject.equals(test2.uid)
				&& d.status == AclWithState.AclStatus.REMOVED && d.entry.verb == Verb.Read));
	}
