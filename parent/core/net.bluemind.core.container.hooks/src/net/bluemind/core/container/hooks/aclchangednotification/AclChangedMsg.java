...
public record AclChangedMsg(String sourceUserId, String domainUid, String containerUid, String containerName,
		String containerType, String containerOwnerDisplayname, List<AclWithState> diff, boolean isItsOwnContainer) {
	...
	private String aclToString() {
		return diff.stream().map(ace -> ace.entry.toString() + ": " + ace.status).collect(Collectors.joining(", "));
	}
}
