...
public class TodoSync<O> extends LoggedContainerDeltaSync<O, VTodo> {

	public TodoSync(BmContext ctx, ContainerDescriptor container, ItemValue<ContainerHierarchyNode> node,
	...
	public static class SyncFactory implements ContainerSync.Factory {
		...
		public <U> ContainerSync forNode(BmContext ctx, ItemValue<ContainerHierarchyNode> node,
			...
			return new TodoSync<U>(ctx, container, node, owner, domain);
		}

	}

	@Override
	protected ReadApis<VTodo> initReadApi() {
		ITodoList calApi = ctx.provider().instance(ITodoList.class, node.value.containerUid);
