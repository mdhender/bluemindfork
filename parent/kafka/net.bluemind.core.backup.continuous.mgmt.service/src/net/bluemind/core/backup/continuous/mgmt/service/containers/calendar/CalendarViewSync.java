...
public class CalendarViewSync<O> extends LoggedContainerDeltaSync<O, CalendarView> {

	public CalendarViewSync(BmContext ctx, ContainerDescriptor container, ItemValue<ContainerHierarchyNode> node,
	...
	public static class SyncFactory implements ContainerSync.Factory {
		...
		public <U> ContainerSync forNode(BmContext ctx, ItemValue<ContainerHierarchyNode> node,
			...
			return new CalendarViewSync<U>(ctx, container, node, owner, domain);
		}

	}

	@Override
	protected ReadApis<CalendarView> initReadApi() {
		IUserCalendarViews calApi = ctx.provider().instance(IUserCalendarViews.class, domain.uid, owner.uid);
