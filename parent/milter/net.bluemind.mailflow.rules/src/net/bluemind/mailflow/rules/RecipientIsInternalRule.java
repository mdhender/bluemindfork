...
public class RecipientIsInternalRule extends DefaultRule implements MailRule {

	@Override
	public String identifier() {
		return "RecipientIsInternalRule";
	}

	@Override
	public String description() {
		return "Checks if all of the recipients are internal";
	}
	...
	public MailRuleEvaluation evaluate(Message message, IClientContext context) {
		...
		if (recipientStream.allMatch(rec -> isInternal(rec, aliases))) {
	...
	private boolean isInternal(String rec, Set<String> aliases) {
		String domain = rec.substring(rec.indexOf("@") + 1);
		return aliases.stream().anyMatch(alias -> domain.equals(alias));
	}
