...
public final class ImageIOUtil {
	 ...
	 * compression is set for maximum compression for PNG and maximum quality for
	 * all other file formats. See
	 * {@link #writeImage(BufferedImage image, String formatName, OutputStream output, int dpi, float compressionQuality)}
	 * for more details.
	 *
	 * @param image    the image to be written
	 * @param filename used to construct the filename for the individual image. Its
	 *                 suffix will be used as the image format.
	 ...
	 *                           image. Its suffix will be used as the image format.
	 * @param dpi                the resolution in dpi (dots per inch) to be used in
	 *                           metadata
	 * @param compressionQuality quality to be used when compressing the image (0
	 *                           &lt; compressionQuality &lt; 1.0f). See
	 *                           {@link ImageWriteParam#setCompressionQuality(float)}
	 *                           for more details.
	 * @return true if the image file was produced, false if there was an error.
	 * @throws IOException if an I/O error occurs
	 */
	public static boolean writeImage(BufferedImage image, String filename, int dpi, float compressionQuality)
			throws IOException {
	 ...
	 *
	 * @param image      the image to be written
	 * @param formatName the target format (ex. "png") which is also the suffix for
	 *                   the filename
	...
	/**
	 * Writes a buffered image to a file using the given image format. Compression
	 * is fixed for PNG, GIF, BMP and WBMP, dependent of the compressionQuality
	 * parameter for JPG, and dependent of bit count for TIFF (a bitonal image will
	 * be compressed with CCITT G4, a color image with LZW). Creating a TIFF image
	 * is only supported if the jai_imageio library (or equivalent) is in the class
	 * path.
	 *
	 * @param image              the image to be written
	 * @param formatName         the target format (ex. "png")
	 ...
	 * parameter for JPG, and dependent of bit count for TIFF (a bitonal image will
	 * be compressed with CCITT G4, a color image with LZW). Creating a TIFF image
	 * is only supported if the jai_imageio library is in the class path.
	 *
	 * @param image              the image to be written
	 * @param formatName         the target format (ex. "png")
	 * @param output             the output stream to be used for writing
	 ...
	 *                           files: If null, save uncompressed; if empty string,
	 *                           use logic explained above; other valid values are
	 *                           found in the javadoc of <a href=
	 *                           "https://download.java.net/media/jai-imageio/javadoc/1.1/com/sun/media/imageio/plugins/tiff/TIFFImageWriteParam.html">TIFFImageWriteParam</a>.
	 * @return true if the image file was produced, false if there was an error.
	 * @throws IOException if an I/O error occurs
	 */
	public static boolean writeImage(BufferedImage image, String formatName, OutputStream output, int dpi,
			float compressionQuality, String compressionType) throws IOException {
	...
	private static void setDPI(IIOMetadata metadata, int dpi, String formatName) throws IIOInvalidTreeException {
		...
		float res = "PNG".equalsIgnoreCase(formatName) ? dpi / 25.4f : 25.4f / dpi;
