...
public class SearchCommand extends AnalyzedCommand {
	...
	private static final Pattern quotedString = Pattern.compile("search (.*)$", Pattern.CASE_INSENSITIVE);
	 ...
	 * @param raw
	 */
	protected SearchCommand(RawImapCommand raw) {
		super(raw);
		...
		if (match.find()) {
			...
			throw new EndpointRuntimeException("unknown search format '" + flat.fullCmd + "'");
		}
	}
