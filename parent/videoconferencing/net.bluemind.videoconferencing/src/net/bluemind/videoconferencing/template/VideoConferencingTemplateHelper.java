/* BEGIN LICENSE
  ...
  * END LICENSE
  */
package net.bluemind.videoconferencing.template;
...
public class VideoConferencingTemplateHelper implements IResourceTemplateHelper {

	private static final Logger logger = LoggerFactory.getLogger(VideoConferencingTemplateHelper.class);

	private static final List<IVideoConferencing> providers = loadProviders();
	...
	private static List<IVideoConferencing> loadProviders() {
		List<IVideoConferencing> all = new RunnableExtensionLoader<IVideoConferencing>()
	...
	public String processTemplate(final String domainUid, final String resourceUid, final String localeLanguageTag,
		...
		Optional<IVideoConferencing> videoConferencingProvider = providers.stream()
		...
		final Map<String, String> props = mapOfProps(resourceDescriptor, organizerName,
				videoConferencingProvider.get().getUrl(settings));
		while (matcher.find()) {
