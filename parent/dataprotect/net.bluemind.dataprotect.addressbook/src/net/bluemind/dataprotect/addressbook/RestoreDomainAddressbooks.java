...
public class RestoreDomainAddressbooks implements IRestoreActionProvider {

	public RestoreDomainAddressbooks() {
	}
	...
	public TaskRef run(RestoreOperation op, DataProtectGeneration backup, Restorable item,
		...
		return tsk.run(new RestoreDomainBooksTask(backup, item));
	}

	@Override
	public List<RestoreOperation> operations() {
		RestoreOperation replace = new RestoreOperation();
		replace.identifier = "replace.domain.books";
		replace.kind = RestorableKind.ADDRESSBOOK;
