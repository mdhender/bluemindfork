...
public class RestoreDomainCalendars implements IRestoreActionProvider {

	public RestoreDomainCalendars() {
	}
	...
	public TaskRef run(RestoreOperation op, DataProtectGeneration backup, Restorable item,
		...
		return tsk.run(new RestoreDomainCalendarsTask(backup, item));
	}

	@Override
	public List<RestoreOperation> operations() {
		RestoreOperation replace = new RestoreOperation();
		replace.identifier = "replace.domain.calendars";
		replace.kind = RestorableKind.CALENDAR;
