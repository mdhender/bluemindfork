 ...
 * END LICENSE
 */
package net.bluemind.dataprotect.smime.cacerts.internal;
...
import net.bluemind.dataprotect.smime.cacerts.impl.RestoreSmimeCacertTask;
...
public class RestoreSmimeCacertsTaskTests {
	...
	@Test
	public void testRestoreDefaultSmimeCacerts() throws Exception {
		ISmimeCACert certs = testContext.provider().instance(ISmimeCACert.class,
				ISmimeCacertUids.domainCreatedCerts(changUid));
		certs.create("test1", defaulSmimeCacert());
		certs.create("test2", defaulSmimeCacert());
		certs.create("test3", defaulSmimeCacert());

		doBackup();
		certs.delete("test2");

		new RestoreSmimeCacertTask(latestGen, restorable).run(new TestMonitor());

		certs = testContext.provider().instance(ISmimeCACert.class, ISmimeCacertUids.domainCreatedCerts(changUid));
		assertEquals(ImmutableSet.of("test1", "test2", "test3"), ImmutableSet.copyOf(certs.allUids()));
	}

	@Test
	public void testRestoreDeletedSmimeCacerts() throws Exception {
		ISmimeCACert certs = testContext.provider().instance(ISmimeCACert.class,
				ISmimeCacertUids.domainCreatedCerts(changUid));
		certs.create("test1", defaulSmimeCacert());
		certs.create("test2", defaulSmimeCacert());
	...
	private SmimeCacert defaulSmimeCacert() {
		SmimeCacert cert = new SmimeCacert();
		cert.cert = "certificate content";
		return cert;
	}
