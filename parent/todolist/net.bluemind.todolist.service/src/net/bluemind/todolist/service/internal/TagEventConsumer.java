...
public class TagEventConsumer implements ITagEventConsumer {
	...
	public void tagChanged(String tagContainerUid, String tagUid) {
		...
		try {
			items = tagReferencesStore.referencedBy(ITodoUids.TYPE, tagContainerUid, tagUid);
		...
		for (ItemUri itemUri : items) {
			...
			if (currentContainer == null || !itemUri.containerUid.equals(currentContainer.uid)) {
				try {
					dsTodos = DataSourceRouter.get(context, itemUri.containerUid);
					ContainerStore containerStore = new ContainerStore(context, dsTodos, SecurityContext.SYSTEM);
		...
		if (tag == null) {
			...
			try {
				tagReferencesStore.deleteReferences(ITodoUids.TYPE, tagContainerUid, tagUid);
			} catch (SQLException e) {
	...
	private void notifyContainerChanged(Container currentContainer) {
		new TodoListEventProducer(currentContainer, SecurityContext.SYSTEM, VertxPlatform.eventBus()).changed();
	}
}
