...
public class OldIncrementalUpdatesAndSearchTests extends BaseIncrementalUpdatesAndSearchTests {
	...
	protected IncrementalProductionStrategy strategy(File dir, CountDownLatch latch) {
		...
		HollowProducer producer = HollowProducer.withPublisher(publisher).withAnnouncer(announcer)
		...
		return (List<Long> toDrop, OfflineAddressBook book, AddressBookRecord... recs) -> {
			...
			long version = incremental.runCycle();
			System.err.println("oldIncremental runCycle returned " + version);
		};
