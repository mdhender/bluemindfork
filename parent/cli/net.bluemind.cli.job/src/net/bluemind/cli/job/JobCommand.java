...
public class JobCommand {
	...
	protected ListResult<Job> getjobs(CliContext ctx, String target) {
		if (target.contains("@")) {
			ctx.error("ERROR: provide a domain or global.virt.");
		} else {
			IDomains domainApi = ctx.adminApi().instance(IDomains.class, target);
			ItemValue<Domain> domain = domainApi.get(target);
			if(domain != null) {
