...
/**
 * Contains the logic shared by {@link RetryableStage} and
 * {@link AsyncRetryableStage} when querying and interacting with a
 * {@link RetryPolicy}.
 */
@SdkInternalApi
public class RetryableStageHelper {
	...
	public RetryableStageHelper(SdkHttpFullRequest request, RequestExecutionContext context,
	...
	/**
	 * Returns true if the retry policy allows this attempt. This will always return
	 * true if the current attempt is not a retry (i.e. it's the first request in
	 * the execution).
	 */
	 ...
	 * This may be {@link Duration#ZERO}, such as for the first request in the
	 * request series.
	 */
	...
	public void logBackingOff(Duration backoffDelay) {
		SdkStandardLogger.REQUEST_LOGGER.debug(() -> "Retryable error detected. Will retry in "
				+ backoffDelay.toMillis() + "ms. Request attempt number " + attemptNumber);
	}
	...
	public void logSendingRequest() {
		SdkStandardLogger.REQUEST_LOGGER
				.debug(() -> (isInitialAttempt() ? "Sending" : "Retrying") + " Request: " + request);
	}

	/**
	 * Adjust the client-side clock skew if the provided response indicates that
	 * there is a large skew between the client and service. This will allow a
	 * retried request to be signed with what is likely to be a more accurate time.
	 */
	 ...
	 * {@link #startingAttempt()} is invoked.
	 */
	 ...
	 * updated whenever {@link #setLastException} is invoked.
	 */
	...
	/**
	 * Update the {@link #getLastException()} value for this helper. This will be
	 * used to determine whether the request should be retried.
	 */
	public void setLastException(Throwable lastException) {
		if (lastException instanceof CompletionException) {
			...
			this.lastException = SdkClientException
					.create("Unable to execute HTTP request: " + lastException.getMessage(), lastException);
		}
	}

	/**
	 * Set the last HTTP response returned by the service. This will be used to
	...
	private RetryPolicyContext retryPolicyContext(boolean isBeforeAttemptSent) {
		return RetryPolicyContext.builder().request(request).originalRequest(context.originalRequest())
				.exception(lastException).retriesAttempted(retriesAttemptedSoFar(isBeforeAttemptSent))
				.executionAttributes(context.executionAttributes())
				.httpStatusCode(lastResponse == null ? null : lastResponse.statusCode()).build();
	}

	/**
	 * Retrieve the number of retries sent so far in the request execution. This
	 * depends on whether or not we've actually sent the request yet during this
	 * attempt.
	 *
	 * Assuming we're executing attempt 3, the number of retries attempted varies
	 * based on whether the request has been sent to the service yet. Before we send
	 * the request, the number of retries is 1 (from attempt 2). After we send the
	 * request, the number of retries is 2 (from attempt 2 and attempt 3).
	 */
