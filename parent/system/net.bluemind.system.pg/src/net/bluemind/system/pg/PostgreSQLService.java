...
public class PostgreSQLService {
	...
	private static final String PG_CONF_PATH = "/etc/postgresql/11/main";
	...
	public void addDataServer(ItemValue<Server> server, String dbName) {
		...
		try {
			...
			nc.writeFile("/tmp/createdb.sh", getCreateDbScript());
		} catch (ServerFault | IOException e) {
			throw new ServerFault("Fail to read install script", e);
		}
		NCUtils.execOrFail(nc, "chmod +x /tmp/createdb.sh");
		...
		String cmd = String.format("/tmp/createdb.sh %s %s %s fr full", dbName, user, password);
		ExitList ret = NCUtils.exec(nc, cmd);
		if (ret.getExitCode() != 0) {
			ret.forEach(l -> logger.error(l));
			throw new ServerFault("Fail to execute command '" + cmd + "'");
		} else {
			ret.forEach(l -> logger.info(l));
		}

		Pool pool;
		try {
			logger.info(".. start pool");
			pool = BMPoolActivator.getDefault().startPool(dbType, user, password, server.value.ip, dbName);
		} catch (Exception e) {
