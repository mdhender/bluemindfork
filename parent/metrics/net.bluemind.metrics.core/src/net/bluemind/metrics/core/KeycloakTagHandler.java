...
public class KeycloakTagHandler extends TickInputConfigurator {
	private static final Logger logger = LoggerFactory.getLogger(KeycloakTagHandler.class);
	...
	public void onServerTagged(BmContext context, ItemValue<Server> itemValue, String tag) throws ServerFault {
		...
		try {
			TagHelper.jarToFS(getClass(), "/configs/bm-keycloak.conf", "/etc/telegraf/telegraf.d/bm-keycloak.conf",
	...
	public void onServerUntagged(BmContext context, ItemValue<Server> itemValue, String tag) throws ServerFault {
		...
		TagHelper.deleteRemote(itemValue.value.address(), "/etc/telegraf/telegraf.d/bm-keycloak.conf");
		logger.info("Deleted file /etc/telegraf/telegraf.d/bm-keycloak.conf at {}", itemValue.value.address());
		TagHelper.reloadTelegraf(itemValue.value.address());
	}
}