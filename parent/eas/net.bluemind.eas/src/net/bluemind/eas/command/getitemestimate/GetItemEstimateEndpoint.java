...
public class GetItemEstimateEndpoint extends WbxmlHandlerBase implements IEasRequestEndpoint {

	private static final Logger logger = LoggerFactory.getLogger(GetItemEstimateEndpoint.class);
	private GetItemEstimateProtocol protocol;

	public GetItemEstimateEndpoint() {
		protocol = new GetItemEstimateProtocol();
	}

	@Override
	public void handle(AuthorizedDeviceQuery dq, Document doc, String userLogin) {
		if (logger.isDebugEnabled()) {
			EasLogUser.logDebugAsUser(userLogin, logger, "GetItemEstimate with protocol...");
		}
