 ...
 * 
 * APPLY UNQUOTA polyclinique-limoges.fr!user.test^2
 *
 */
public class ApplyUnquota implements IAsyncReplicationCommand {

	private static final Logger logger = LoggerFactory.getLogger(ApplyUnquota.class);
	...
	public CompletableFuture<CommandResult> doIt(ReplicationSession session, Token t, ReplicationFrame frame) {
		...
		String toDelete = withVerb.substring("APPLY UNQUOTA ".length());
		logger.info("UNQUOTA just says ok, request was for '{}'", toDelete);
