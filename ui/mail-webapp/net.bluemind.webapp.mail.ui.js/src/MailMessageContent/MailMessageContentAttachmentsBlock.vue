        ...
        <bm-row class="mb-2">
            <bm-col cols="12" class="pl-2">
                <bm-icon 
                    ...
                    @click="toggleExpand"
                />
                <bm-icon icon="paper-clip" class="mr-1" size="lg" :class="hasMoreThan3Attachments ? 'ml-2': 'ml-4'" />
                <span class="font-weight-bold pr-2">
                    {{ $tc("common.attachments", attachments.length, { count: attachments.length }) }}
                </span> 
            </bm-col>
        </bm-row>
        <bm-row v-if="shouldProposeExpandedMode" class="ml-2 mr-1">
            <bm-col cols="4">
            ...
            <bm-col cols="4" class="mb-2">
                <bm-button 
                    ...
                    class="w-100"
                    :class="hasPreview(attachments[0]) || hasPreview(attachments[1]) ? 'h-25' : 'h-100'"
                    @click="toggleExpand"
                >
