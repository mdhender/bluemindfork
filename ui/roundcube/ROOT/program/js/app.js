...
function rcube_webmail()
{
        ...
        if (this.gui_objects.qsearchbox) {
          if (this.env.search_text != null) {
            this.gui_objects.qsearchbox.value = this.env.search_text;
      	    $(this.gui_objects.qsearchbox).change();
          }
  ...
  this.command = function(command, props, obj)
  {
    ...
    switch (command) {
      ...
      case 'load-attachment':
        ...
        this.goto_url('get', qstring+'', false);
        break;
        ...
        if (li = this.get_folder_li(k)) {
          ...
          if (height = li.firstChild.offsetHeight) {
            ...
            this.env.folder_coords[k] = { x1:pos.left, y1:pos.top,
              x2:pos.left + list.width(), y2:pos.top + height, on:0 };
          }
        }
      }
    }
  ...
  this.collapse_folder = function(name)
  {
    var li = this.get_folder_li(name, '', true),
      div = $('div:first', li),
      ul = $('ul:first', li),
      caret = $('span.caret:first', li);
          
    if (div.hasClass('collapsed')) {                                                                                                   
      ul.show();
      if (!caret.hasClass('fa-inbox')){
        caret.removeClass('fa fa-folder').addClass('fa fa-folder-open');
      }       
      ...
      ul.hide();
      if (!caret.hasClass('fa-inbox')){
        caret.removeClass('fa fa-folder-open').addClass('fa fa-folder');
      } 
  ...
  this.add_message_row = function(uid, cols, flags, attop)
  {
    ...
    answered = '<span class="status">&nbsp;</span>';
    if (flags.answered)
      answered = '<span class="fa fa-reply">&nbsp;</span>'

    forwarded = '<span class="status">&nbsp;</span>';
    if (flags.forwarded)
      forwarded = '<span class="fa fa-long-arrow-right">&nbsp;</span>'
    ...
    if (this.env.threading) {
      if (message.depth) {
        ...
        expando = '<div id="rcmexpando' + uid + '" class="' + (message.expanded ? 'fa fa-lg fa-caret-square-o-down expanded' : 'fa fa-lg fa-caret-square-o-right collapsed') + '">&nbsp;&nbsp;</div>';
    ...
    for (n in this.env.coltypes) {
      ...
      if (c == 'subject') {
        ...
        html = '<span id="flagicn'+uid+'" class="fa fa-flag '+css_class+'">&nbsp;</span>';
      }
        ...
        html = '<span id="statusicn'+uid+'" class="fa fa-circle '+css_class+'">&nbsp;</span>';
      }
      else if (c == 'threads')
        html = expando;
      else if (c == 'priority') {
        ...
        if (!r.depth) { // a new root
          ...
          if (r.has_children) {
            ...
            $('#rcmrow'+r.uid+' '+'.leaf:first')
              ...
              .attr('class', (r.obj.style.display != 'none' ? 'fa fa-lg fa-caret-square-o-down expanded' : 'fa fa-lg fa-caret-square-o-right collapsed'))
  ...
  this.set_message_icon = function(uid)
  {
    ...
    if (row.icon) {
      css_class = 'fa fa-circle msgicon';
      if (row.deleted)
        css_class += ' deleted';
      else if (row.unread)
        css_class += ' unread';
    ...
    if (row.flagicon) {
      css_class = (row.flagged ? 'fa fa-flag flagged' : 'fa fa-flag unflagged');
  ...
  this.delete_messages = function()
  {
    ...
    // BM-5623
    if (this.env.quota_content.percent == 100) {
      if (window.confirm(this.get_label('quotafulldeletemessagesconfirm'))) {
  ...
  this._with_selected_messages = function(action, lock, add_url)
  {
    ...
    if (this.env.uid)
      ...
      for (n=0, len=selection.length; n<len; n++) {
        ...
        if (!next_row || next_row.uid == id || !this.message_list.rows[next_row.uid]) { 
          next_row = this.message_list.get_next_row() ||this.message_list.get_prev_row();
          if (next_row) {
            this.message_list.last_selected = next_row.uid;
          }
        }
        this.message_list.remove_row(id, false);

      }
      if (this.env.display_next && next_row) {
        this.message_list.select_row(next_row.uid, false, false);
      }
  ...
  this.init_address_input_events = function(obj, props)
  {
    ...
    obj[bw.ie || bw.safari || bw.chrome || bw.mz ? 'keydown' : 'keypress'](function(e) { return ref.ksearch_keydown(e, this, props); })
    ...
    if (!is_html) {
      ...
      if (!sigElem) {
        ...
        if (this.env.sig_above) {
          ...
          node = editor.dom.getParent(node, function(el) { return editor.dom.isBlock(el);});
      ...
      else if (sig && this.env.signatures && this.env.signatures[sig])
        sigElem.innerHTML = '';
    }
  ...
  this.set_unread_count_display = function(mbox, set_title)
  {
    ...
    if (set_title && document.title) {
      ...
      if (mycount && doc_title.match(reg))
        ...
        new_title = doc_title.replace('-', '('+mycount+') -');
      } else {
  ...
  this.editor_init = function(config)
  {
    this.editor = new rcmail_text_editor(config);
  };
  ...
  this.html2plain = function(htmlText, callback)
  {
    var rcmail = this,
    ...
    $.ajax({ type: 'POST', url: url, data: htmlText, contentType: 'application/octet-stream',
      ...
      success: function(data) { rcmail.set_busy(false, null, lock); callback(data); rcmail.log(data); }
    });
    return true;
  };

  this.plain2html = function(plain, callback)
  {
    var lock = this.set_busy(true, 'converting');

    plain = plain.replace(/&/g, '&amp;').replace(/</g, '&lt;').replace(/>/g, '&gt;');
    callback(plain ? '<pre>' + plain + '</pre>' : plain );
