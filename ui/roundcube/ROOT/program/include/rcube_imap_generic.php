        ...
        if (is_array($value)){
            if ($decode) {
                foreach($value as $i => $val) {
                    $value[$i] = rcube_mime::decode_header($val, $this->charset);
                } 
            }
            return $value;
        }
...
*/
                        ...
                        default:
                            if (strlen($field) > 2) {
                                if (isset($result[$id]->others[$field])){
                                    if (!is_array($result[$id]->others[$field])){
                                        $result[$id]->others[$field] = array( $result[$id]->others[$field] );
                                    } 
                                    array_push($result[$id]->others[$field],$string);
                                } else {
    ...
    function fetchMIMEHeaders($mailbox, $uid, $parts, $mime=true)
    {
        ...
        do {
            ...
            $line = preg_replace('/^\* [0-9]+ FETCH [0-9UID( ]+/', '', $line);
            if (preg_match('/^\s*BODY\[([0-9\.]+)\.'.$type.'\]/', $line, $matches)) {
                $idx     = $matches[1];
                $headers = '';
