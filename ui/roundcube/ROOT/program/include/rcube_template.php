    ...
    public function button($attrib)
    {
        ...
        if ($attrib['type']=='image') {
            ...
            if ($attrib['innerclass']) {
                if (strpos($attrib['innerclass'], "fa-") !== false) {
                  $contentClass = "iconized";
                  $klasses = explode(" ", $attrib['innerclass']);
                  for($i = 0; $i<sizeof($klasses);$i++) {
                    if ($klasses[$i] != "fa" && substr($klasses[$i], 0, 3) !== "fa-") {
                      $contentClass .= " ".$klasses[$i];
                    }
                  }
                  


                  $btn_content = html::span($attrib['innerclass']." iconized", "").html::span($contentClass, $btn_content);
                } else {
