...
public class GroupMenus {
	...
	public static MenuContribution getContribution() {
		GroupMenusConstants v = GWT.create(GroupMenusConstants.class);
		JsArray<Contributed<Section>> sections = JsArray.createArray().cast();
		JsArray<Contributed<Screen>> screens = JsArray.createArray().cast();

		screens.push(
				Contributed.create("directories", Screen.createDirEditor("editGroup", v.editGroup(), null, false)));

		screens.push(
				Contributed.create("dir1", Screen.create("qcGroup", v.qcGroup(), BasicRoles.ROLE_MANAGE_GROUP, true)
						.withOURoles(BasicRoles.ROLE_MANAGE_GROUP)));
