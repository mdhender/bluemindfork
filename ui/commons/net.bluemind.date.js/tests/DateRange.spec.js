...
describe("DateRange", () => {
    ...
    test.skip("this week range to match current week (expect this to fail)", () => {
        const today = new Date("2019-01-01");
        // Missing fdow calculation
        expect(DateRange.thisWeek(today).contains(new Date("2018-12-30"))).toBe(false);
        expect(DateRange.thisWeek(today).contains(new Date("2018-12-31"))).toBe(true);
        expect(DateRange.thisWeek(today).contains(new Date("2019-01-06"))).toBe(true);
        expect(DateRange.thisWeek(today).contains(new Date("2019-01-07"))).toBe(false);
    }),
