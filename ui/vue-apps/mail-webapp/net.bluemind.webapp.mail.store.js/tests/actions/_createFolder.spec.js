...
describe("[Mail-WebappStore][actions] :  $_createFolder", () => {
    ...
    test("do not create folder if path already exists", done => {
        ...
        let promise = context.getters["folders/folders"].reduce((promise, f) => {
            ...
            return promise.then(() => $_createFolder(context, { folder, mailboxUid }));
        }, Promise.resolve());
        promise.then(() => {
    ...
    test("do not create folder if a key is provided", done => {
        ...
        let promise = context.getters["folders/folders"].reduce((promise, f) => {
            ...
            return promise.then(() =>
                $_createFolder(context, { folder, mailboxUid }).then(key => expect(key).toEqual(folder.key))
            );
        }, Promise.resolve());
        promise.then(() => {
            expect(context.dispatch).not.toHaveBeenCalled();
    ...
    test("create folder and return the folder key", done => {
        ...
        $_createFolder(context, { folder, mailboxUid }).then(key => {
    ...
    test("create folder within the correct hierarchy", done => {
        ...
        $_createFolder(context, { folder, mailboxUid })
            .then(() => {
                expect(context.dispatch).toHaveBeenCalledWith("folders/create", {
                    name: "again",
                ...
                return $_createFolder(context, { folder, mailboxUid });
            })
            .then(() => {
                expect(context.dispatch).toHaveBeenCalledWith("folders/create", {
                    name: "things",
    ...
    test("create all folder's parents", done => {
        let folders = createHierarchy("My", mailboxUid);
        context.getters["folders/folders"] = folders;
        let folder = { value: { fullName: "Another/one/to", name: "again" } };

        context.dispatch.mockReturnValueOnce(Promise.resolve(ItemUri.encode("key1", mailboxUid)));
        context.dispatch.mockReturnValueOnce(Promise.resolve(ItemUri.encode("key2", mailboxUid)));
        context.dispatch.mockReturnValueOnce(Promise.resolve(ItemUri.encode("key3", mailboxUid)));
        context.dispatch.mockReturnValueOnce(Promise.resolve(ItemUri.encode("key4", mailboxUid)));
        context.dispatch.mockReturnValueOnce(Promise.resolve(ItemUri.encode("key5", mailboxUid)));
        ...
        $_createFolder(context, { folder, mailboxUid })
            .then(key => {
                ...
                expect(key).toEqual(ItemUri.encode("key3", mailboxUid));
            ...
            .then(key => {
                ...
                expect(key).toEqual(ItemUri.encode("key5", mailboxUid));
                done();
            });
    });
    test("create folders in another mailbox", done => {
        ...
        $_createFolder(context, { folder, mailboxUid: "another-mailbox-uid" }).then(() => {
            expect(context.dispatch).toHaveBeenCalledWith("folders/create", {
                name: "folder",
