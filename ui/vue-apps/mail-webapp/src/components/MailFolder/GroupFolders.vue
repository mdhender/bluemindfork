<template>
    ...
    <mail-folder-tree
        ...
        :tree="GROUP_MAILBOXES_ROOT_FOLDERS"
        :expanded="expanded"
...
import { MAILBOXES_ARE_LOADED, GROUP_MAILBOXES_ROOT_FOLDERS, GROUP_MAILBOXES } from "~/getters";
...
export default {
    ...
    computed: {
        ...mapGetters("mail", { GROUP_MAILBOXES_ROOT_FOLDERS, GROUP_MAILBOXES, MAILBOXES_ARE_LOADED }),
        isLoaded() {
            return (
                this.MAILBOXES_ARE_LOADED && (!this.GROUP_MAILBOXES.length || this.GROUP_MAILBOXES_ROOT_FOLDERS.length)
            );
        }
    }
};
