...
export default {
    ...
    computed: {
        ...
        contacts() {
            return this.isSearching ? this.searchedContacts : this.allContacts;
        },
        isSearching() {
        ...
        bookAndPageKey() {
            return `${this.selectedAddressBookId}-${this.currentPage}`;
        }
    },
    watch: {
    ...
    methods: {
        ...
        async performSearch(searchValue) {
            if (searchValue) {
                ...
                const queryObject = searchVCardsHelper(searchValue, {
                    size: 50,
                    noGroup: false,
                    addressBook: this.selectedAddressBookId,
                    fields: [SearchFields.EMAIL, SearchFields.NAME, SearchFields.COMPANY]
                });
        ...
        async loadContacts() {
            ...
            this.loading = true;
            try {
                if (this.selectedAddressBookId) {
                    const { resultsInPage, pageStartIndex, totalLength } = await apiAddressbooks.byPage(
                        this.currentPage,
                        this.perPage,
                        this.selectedAddressBookId
                    );
                    this.allContacts = buildAllContacts(
                        this.selectedAddressBookId,
                        this.perPage,
                        resultsInPage,
                        pageStartIndex,
