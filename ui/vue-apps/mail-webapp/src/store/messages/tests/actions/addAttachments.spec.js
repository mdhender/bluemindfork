...
describe("addAttachments action", () => {
    ...
    test("With error", async () => {
        ...
        expect(context.commit).toHaveBeenNthCalledWith(
            2,
            mutationTypes.SET_ATTACHMENT_PROGRESS,
            expect.objectContaining({ messageKey, loaded: 100, total: 100 })
        );
        expect(context.commit).toHaveBeenNthCalledWith(
            3,
            mutationTypes.SET_ATTACHMENT_STATUS,
    ...
    test("Cancelled by user", async () => {
        ...
        expect(context.commit).toHaveBeenNthCalledWith(1, mutationTypes.ADD_ATTACHMENT, expect.anything());
        jest.runAllTimers();
        expect(context.commit).toHaveBeenNthCalledWith(2, mutationTypes.REMOVE_ATTACHMENT, expect.anything());
    });
});
